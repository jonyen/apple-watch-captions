...
function harness(options: { twoWay?: boolean } = {}) {
  ...
  handleTwilioStream(ws, store, calls, downlink, uplink, "user-a", options);
...
describe("handleTwilioStream", () => {
  ...
  it("does not resurrect a replaced call's session or leak its provider", () => {
    ...
    handleTwilioStream(wsA, store, calls, downlink, uplink, "user-a");
    handleTwilioStream(wsB, store, calls, downlink, uplink, "user-a");
  ...
  // Newest-call-wins is scoped per user: another user's start frame beginning
  // THEIR call must not evict, close, or wipe this user's — the base supports
  // two users' concurrent calls (see server.tenancy.test.ts), and the audio
  // maps must not quietly reintroduce the single-slot eviction the keyed
  // CurrentCall removed.
  it("does not let one user's start frame displace another user's live call", () => {
    ...
    const wsB = new FakeSocket();
    handleTwilioStream(wsA, store, calls, downlink, uplink, "user-a");
    handleTwilioStream(wsB, store, calls, downlink, uplink, "user-b");

    wsA.receive(startFrame("CA-a"));
    wsA.receive(mediaFrame("AAECAw==")); // a's caller audio, waiting for a's watch

    wsB.receive(startFrame("CA-b"));

    // a's call is untouched: still current, socket open, audio still waiting.
    expect(calls.current("user-a")?.sessionId).toBe("CA-a");
    expect(wsA.closes).toBe(0);
    expect([...downlink.drain("user-a", 0).audio]).toEqual([0, 1, 2, 3]);
    // And b's caller audio lands in b's compartment, not a's.
    wsB.receive(mediaFrame("BAU="));
    expect([...downlink.drain("user-b", 0).audio]).toEqual([4, 5]);
    expect([...downlink.drain("user-a", 1).audio]).toEqual([]);
    // Both uplinks are live at once.
...
describe("bidirectional audio", () => {
  ...
  it("sends uplink audio back to Twilio addressed to the stream", () => {
    ...
    expect(uplink.write("user-a", Buffer.from([0xff, 0xff]))).toBe(true);
  ...
  it("stops accepting uplink audio once the call ends", () => {
    ...
    expect(uplink.write("user-a", Buffer.from([0xff]))).toBe(false);
  });
  ...
  it("empties the downlink buffer when a call ends, so the next call starts clean", () => {
    ...
    expect(downlink.drain("user-a", 0).audio.length).toBe(0);
  });
  ...
  it("does not let a stale socket's teardown silence a newer call's uplink", () => {
    ...
    expect(uplink.write("user-a", Buffer.from([0xaa]))).toBe(true);
